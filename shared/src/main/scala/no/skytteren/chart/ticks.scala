...
object ticks {
  ...
  def apply(start: BigDecimal, stop: BigDecimal, count: Int): List[Double] = {
    ...
    if (min == max && count > 0) {
      ...
      val steps: List[BigDecimal] =
        if (step == 0d || step.toDouble.isInfinity) {
          Nil
        } else {
          if (step > 0) {
      ...
      (if (reverse) steps.reverse else steps).map(_.toDouble)
    }
  }
  ...
  def increment(start: BigDecimal, stop: BigDecimal, count: Int): Double = {
    ...
    val value: Double = if (error >= e10) 10 else if (error >= e5) 5 else if (error >= e2) 2 else 1
  ...
  def tickStep(start: BigDecimal, stop: BigDecimal, count: Int): BigDecimal = {
    ...
    val stepping: Double = if (error >= e10) step1 * 10 else if (error >= e5) step1 * 5 else if (error >= e2) step1 * 2 else step1
    if(stop < start) -stepping else stepping
  }
