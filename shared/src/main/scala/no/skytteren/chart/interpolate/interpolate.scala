...
object Interpolater:
  ...
  type Factory[N, O[_]] = O[N] => Interpolater[N]
...
case class InterpolateRound[N: NumberData](range: StartEndRange[N]) extends Interpolater[N]:
  private val numeric = implicitly[NumberData[N]]
  def apply(value: Double): N =
    numeric.reverse((numeric(range.start) * (1d - value) + numeric(range.end) * value).round.toDouble)

  def span: Option[Double] = Some(numeric(range.end) - numeric(range.start))
...
case class InterpolateDate(range: StartEndRange[Date]) extends Interpolater[Date]:
  ...
  override def clamp(d: Date): Date =
    if d < range.start then
      range.start
    else if d > range.end then
      range.end
    else
      d

  override def span: Option[Double] = Some(range.end.toDays.value - range.start.toDays.value.toDouble)
...
case class InterpolateDateTime(range: StartEndRange[DateTime]) extends Interpolater[DateTime]{
  ...
  override def clamp(d: DateTime): DateTime =
    if d < range.start then
      range.start
    else if range.end < d then
      range.end
    else
      d

  override def span: Option[Double] = Some(range.end.toEpochSecond.value - range.start.toEpochSecond.value.toDouble)
}